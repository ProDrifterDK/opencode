...
describe("RateLimiter", () => {
  test("acquire proceeds immediately when under budget", async () => {
    const result = await runWith(Effect.gen(function* () {
      ...
      yield* service.acquire(ENG_A, "engineer", 1000)
  ...
  test("priority ordering — lead goes before engineer", async () => {
    const result = await runWithScope(Effect.gen(function* () {
      ...
      yield* service.acquire(ENG_A, "engineer", 100).pipe(
        Effect.tap(() => Effect.sync(() => order.push("engineer"))),
        Effect.forkScoped,
      )
      yield* service.acquire(ENG_LEAD, "lead", 100).pipe(
        Effect.tap(() => Effect.sync(() => order.push("lead"))),
        Effect.forkScoped,
      )

      yield* Effect.sleep(5)
      yield* service.release(`eng_fill_0` as EngineerID, 100)
  ...
  test("resetCircuitBreaker clears breaker state", async () => {
    const result = await runWith(Effect.gen(function* () {
      ...
      yield* service.resetCircuitBreaker()
      return service.getStats()
    }))
  ...
  test("release processes queued requests", async () => {
    const result = await runWithScope(Effect.gen(function* () {
      ...
      yield* service.acquire(ENG_A, "engineer", 100).pipe(Effect.forkScoped)
      yield* Effect.sleep(5)
      expect(service.getStats().queuedCalls).toBe(1)
  ...
  test("report429 with <3 failures does not disrupt queued requests", async () => {
    const result = await runWithScope(
      Effect.gen(function* () {
        ...
        yield* service.report429(ENG_C)
        expect(service.getStats().consecutive429s).toBe(1)
        expect(service.getStats().queuedCalls).toBe(1)
        expect(service.getStats().circuitBreakerOpen).toBe(false)
  ...
  test("getStats returns correct snapshot", async () => {
    const result = await runWith(Effect.gen(function* () {
      const service = yield* RateLimiterService
      yield* service.acquire(ENG_A, "engineer", 5000)
      yield* service.acquire(ENG_B, "lead", 3000)
      return service.getStats()
    }))
  ...
  test("fair scheduling within same priority — FIFO order", async () => {
    const result = await runWithScope(Effect.gen(function* () {
      ...
      yield* service.acquire(ENG_B, "engineer", 100).pipe(
      ...
      yield* service.release(`eng_fill_0` as EngineerID, 100)
      yield* Effect.sleep(10)
      yield* service.release(`eng_fill_1` as EngineerID, 100)
      yield* Effect.sleep(10)
  ...
  test("circuit breaker fails all queued requests", async () => {
    const result = await runWithScope(
      Effect.gen(function* () {
        ...
        const fiberA = yield* service.acquire(ENG_A, "engineer", 100).pipe(
          Effect.flip,
          Effect.forkScoped,
        )
        const fiberB = yield* service.acquire(ENG_B, "lead", 100).pipe(
          Effect.flip,
          Effect.forkScoped,
        )
