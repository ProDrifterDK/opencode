...
export const buildEngineerPrompt = (
  ...
  return [
    ...
    `   - summary: ONE sentence + path to report file (keep under 200 chars)`,
    `   - reportPath: ".tmp/report-${input.name}.md"`,
    `   - changedFiles: files you changed or inspected as final evidence`,
    `   - verificationCommands: exact commands run, with outcomes in the report file`,
    `   - knownGaps: honest gaps, skipped checks, weak tests, or assumptions`,
    `   - confidence: "low" | "medium" | "high"`,
    `   - DO NOT send full report content via team_report`,
