...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.loop")(function* (
      ...
      // runLoop's body is `while (true)` with `return yield*` as the only
      // exit; TS can't prove the loop never falls through after the
      // mailbox-injection branch, so its inferred return widens to
      // `WithParts | undefined`. Narrow with `!` — runLoop never returns
      // a regular `undefined` value, only via thrown errors / Effect
      // failures which are handled separately by the runtime.
      return yield* state.ensureRunning(input.sessionID, lastAssistant(input.sessionID), runLoop(input.sessionID) as Effect.Effect<MessageV2.WithParts, never, never>)
    })
